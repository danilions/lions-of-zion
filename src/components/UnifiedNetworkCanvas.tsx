...
export default function UnifiedNetworkCanvas() {
  ...
  const initializeFlowingPoints = useCallback(
    (width: number, height: number) => {
      flowingPointsRef.current = Array.from(
        { length: FLOWING_POINTS_COUNT },
        () => ({
          x: Math.random() * width,
          y: Math.random() * height,
  ...
  const drawNetwork = useCallback((ctx: CanvasRenderingContext2D) => {
      ...
      node.connections.forEach((connectionIndex) => {
        if (connectionIndex > i) {
          ...
          gradient.addColorStop(
            0,
            `rgba(0, 174, 239, ${alpha * node.intensity})`,
          );
          gradient.addColorStop(0.5, `rgba(77, 200, 240, ${alpha * 0.8})`);
          gradient.addColorStop(
            1,
            `rgba(0, 174, 239, ${alpha * otherNode.intensity})`,
          );
    ...
    nodesRef.current.forEach((node) => {
      ...
      glowGradient.addColorStop(
        0,
        `rgba(0, 174, 239, ${node.intensity * 0.9})`,
      );
      glowGradient.addColorStop(
        0.3,
        `rgba(77, 200, 240, ${node.intensity * 0.5})`,
      );
      glowGradient.addColorStop(
        0.7,
        `rgba(0, 174, 239, ${node.intensity * 0.2})`,
      );
      ...
      midGradient.addColorStop(
        0,
        `rgba(77, 200, 240, ${node.intensity * 0.8})`,
      );
      midGradient.addColorStop(
        0.4,
        `rgba(0, 174, 239, ${node.intensity * 0.6})`,
      );
      ...
      innerGradient.addColorStop(
        0.7,
        `rgba(77, 200, 240, ${node.intensity * 0.7})`,
      );
      innerGradient.addColorStop(
        1,
        `rgba(0, 174, 239, ${node.intensity * 0.3})`,
      );
  ...
  const drawFlowingPoints = useCallback((ctx: CanvasRenderingContext2D) => {
    flowingPointsRef.current.forEach((point) => {
      ...
      point.trail.forEach((segment, i) => {
        ...
        const trailMidGradient = ctx.createRadialGradient(
          segment.x,
          segment.y,
          0,
          segment.x,
        ...
        trailMidGradient.addColorStop(
          0.6,
          `rgba(77, 200, 240, ${alpha * 0.6})`,
        );
      ...
      const pulseSize =
        mainSize + Math.sin(timeRef.current * 0.08 + point.x * 0.005) * 4;
      ...
      outerGlowGradient.addColorStop(
        0,
        `rgba(0, 174, 239, ${point.intensity * 0.4})`,
      );
      outerGlowGradient.addColorStop(
        0.5,
        `rgba(0, 174, 239, ${point.intensity * 0.2})`,
      );
      ...
      mainGradient.addColorStop(
        0.3,
        `rgba(0, 174, 239, ${point.intensity * 0.8})`,
      );
      mainGradient.addColorStop(
        0.7,
        `rgba(0, 174, 239, ${point.intensity * 0.4})`,
      ...
      innerGradient.addColorStop(
        0.5,
        `rgba(128, 230, 255, ${point.intensity * 0.8})`,
      );
      innerGradient.addColorStop(
        1,
        `rgba(77, 200, 240, ${point.intensity * 0.4})`,
      );
